...
async function rewriteHtml(html, baseUrl) {
  ...
  // Prevent the target page from navigating the top-level proxy site away.
  $("base").remove();
  $("head").prepend(
...
app.get("/proxy", async (req, res) => {
  ...
  try {
    ...
    // Stream images, video, fonts, JavaScript, downloads, etc.
    if (upstream.body) {
...
app.listen(PORT, () => {
  // Intentionally no startup URL or request logging.
});
